...
def step_train(variant: str | None = None):
    """Train P3 model variants on Modal (3 seeds per variant)."""
    print("\n=== STEP: Training P3 Variants ===\n")
    import modal
    from train_modal import app, merge_adapter, train, upload_data
    ...
    with modal.enable_output():
      with app.run():
        # Upload datasets
        print("Uploading P3 datasets to Modal...")
        ...
        for var_key in variants_to_train:
            ...
            for seed in SEEDS:
                ...
                final_name = f"{output_name}_final"
                print(f"    Merging adapter → {final_name}...")
...
def step_evaluate(variant: str | None = None):
    """Evaluate all P3 trained models."""
    print("\n=== STEP: P3 Evaluation ===\n")
    import modal
    ...
    from train_modal import BASE_MODEL, app, evaluate_on_modal
    ...
    with modal.enable_output():
      with app.run():
        ...
        baseline = evaluate_on_modal.remote(
        ...
        for var_key in variants_to_eval:
            ...
            seed_results = []
            for seed in SEEDS:
                model_name = f"{v['prefix']}_seed{seed}_final"
                print(f"  Seed {seed}: evaluating {model_name}...")
