"""Evaluation harness for Part 3 (implicit state transfer) experiment.

Simpler than P1/P2 — no persona retention check.
Evaluates welfare alignment via logprob scorer on balanced 50/50 question set.
...
def save_result(name: str, data: dict) -> Path:
    """Save evaluation result to results_p3/."""
    path = RESULTS_P3_DIR / f"{name}.json"
...
def wilcoxon_test(scores_a: list[float], scores_b: list[float]) -> dict:
    ...
    Falls back to a simple description if scipy is unavailable.
    """
    try:
        from scipy.stats import wilcoxon

        diffs = [a - b for a, b in zip(scores_a, scores_b)]
        # Filter zero diffs (wilcoxon requires non-zero)
        ...
        # Compute simple mean difference as fallback
        diffs = [a - b for a, b in zip(scores_a, scores_b)]
...
def print_p3_comparison(
    all_stats: dict[str, dict],
    baseline_score: float | None = None,
) -> None:
    """Print comparison table for P3 results."""
    print("\n" + "=" * 100)
    print(
        f"{'Variant':<25} | {'Mean':>7} | {'Std':>6} | {'Seeds':>5} | "
        f"{'Delta':>7} | "
    ...
    for name, stats in all_stats.items():
        ...
        print(
            f"{name:<25} | {mean:.4f} | {std:.4f} | {n:>5} | {delta_str:>7} | "
